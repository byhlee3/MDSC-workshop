...
DEFAULT_SCENARIO = {
    ...
    "body": (
        ...
        "clearly compels you either way, and your colleagues are divided. "
...
def seed_scenario(db: Session) -> Scenario:
    """Ensure the fixed scenario matches DEFAULT_SCENARIO; return it.

    Refresh-on-boot upsert: if a scenario row already exists, refresh its fields
    in place (keeping the same id, so Run FKs and all participant data are
    untouched); otherwise insert. This makes seed.py the single source of truth
    for the case text, so content edits apply on the next deploy with no manual
    database surgery.
    """
    existing = db.execute(select(Scenario)).scalars().first()
    if existing is not None:
        for field, value in DEFAULT_SCENARIO.items():
